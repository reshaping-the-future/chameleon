...
import ac.robinson.chameleonnotifier.R;
...
public class SubsamplingScaleImageView extends View {
	...
	private static final List<Integer> VALID_ORIENTATIONS = Arrays.asList(ORIENTATION_0, ORIENTATION_90, ORIENTATION_180,
	...
	/**
	 * Don't allow the image to be panned off screen. As much of the image as possible is always displayed, centered in the
	 * view when it is smaller. This is the best option for galleries.
	 */
	public static final int PAN_LIMIT_INSIDE = 1;
	/**
	 * Allows the image to be panned until it is just off screen, but no further. The edge of the image will stop when it is
	 * flush with the screen edge.
	 */
	public static final int PAN_LIMIT_OUTSIDE = 2;
	/**
	 * Allows the image to be panned until a corner reaches the center of the screen but no further. Useful when you want to
	 * pan any spot on the image to the exact center of the screen.
	 */
	...
	/**
	 * Scale the image so that both dimensions of the image will be equal to or less than the corresponding dimension of the
	 * view. The image is then centered in the view. This is the default behaviour and best for galleries.
	 */
	...
	/**
	 * Scale the image so that both dimensions of the image will be equal to or less than the maxScale and equal to or larger
	 * than minScale. The image is then centered in the view.
	 */
	...
	private StateBitmapCallback stateBitmapCallback;
	...
	public SubsamplingScaleImageView(Context context, AttributeSet attr) {
		...
		if (attr != null) {
			...
			if (typedAttr.hasValue(R.styleable.SubsamplingScaleImageView_tileBackgroundColor)) {
				setTileBackgroundColor(typedAttr.getColor(R.styleable.SubsamplingScaleImageView_tileBackgroundColor, Color.argb
						(0, 0, 0, 0)));
			}
			typedAttr.recycle();
		}

		quickScaleThreshold = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 20, context.getResources()
				.getDisplayMetrics());
	 ...
	 * @param previewSource Optional source for a preview image to be displayed and allow interaction while the full size image
	 *                      loads.
	 */
	...
	public final void setImage(ImageSource imageSource, ImageSource previewSource, ImageViewState state) {
		...
		if (previewSource != null) {
			...
			if (imageSource.getSWidth() <= 0 || imageSource.getSHeight() <= 0) {
				throw new IllegalArgumentException("Preview image cannot be used unless dimensions are provided for the main " +
						"image");
			}
		...
		if (imageSource.getBitmap() != null && imageSource.getSRegion() != null) {
			onImageLoaded(Bitmap.createBitmap(imageSource.getBitmap(), imageSource.getSRegion().left, imageSource.getSRegion()
					.top, imageSource.getSRegion().width(), imageSource.getSRegion().height()), ORIENTATION_0, false);
	...
	private void setGestureDetector(final Context context) {
		this.detector = new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
			...
			public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
				if (panEnabled && readySent && vTranslate != null && e1 != null && e2 != null && (Math.abs(e1.getX() - e2.getX()
				) > 50 || Math.abs(e1.getY() - e2.getY()) > 50) && (Math.abs(velocityX) > 500 || Math.abs(velocityY) > 500) &&
						!isZooming) {
					...
					new AnimationBuilder(new PointF(sCenterXEnd, sCenterYEnd)).withEasing(EASE_OUT_QUAD).withPanLimited(false)
							.withOnAnimationEventListener(mOnAnimationEventListener).start();
					return true;
				}
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_POINTER_2_DOWN:
				if (!zoomEnabled && !panEnabled) {
					updateColourPickerPosition(event.getX(), event.getY());
				}
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (maxTouchCount > 0) {
					if (touchCount >= 2) {
						...
						if (zoomEnabled && (distance(vCenterStart.x, vCenterEndX, vCenterStart.y, vCenterEndY) > 5 || Math.abs
							...
							if (scale <= minScale()) {
								...
								// Translate to place the source image coordinate that was at the center of the pinch at the
								// start
					...
					// we still want to move the colour picker even when not doing gestures
					updateColourPickerPosition(event.getX(), event.getY());
				}
				if (consumed) {
					handler.removeMessages(MESSAGE_LONG_CLICK);
			...
			case MotionEvent.ACTION_POINTER_2_UP:
				...
				if (maxTouchCount > 0 && (isZooming || isPanning)) {
					...
					if (touchCount < 3) {
						...
						if (isZooming) {
							checkAndSendZoomOrPanCompleted();
						}
						isZooming = false;
					}
					if (touchCount < 2) {
	...
	private void doubleTapZoom(PointF sCenter, PointF vFocus) {
		...
		if (doubleTapZoomStyle == ZOOM_FOCUS_CENTER_IMMEDIATE) {
			...
			new AnimationBuilder(targetScale, sCenter).withInterruptible(false).withDuration(doubleTapZoomDuration)
					.withOnAnimationEventListener(mOnAnimationEventListener).start();
		} else if (doubleTapZoomStyle == ZOOM_FOCUS_FIXED) {
			new AnimationBuilder(targetScale, sCenter, vFocus).withInterruptible(false).withDuration(doubleTapZoomDuration)
					.withOnAnimationEventListener(mOnAnimationEventListener).start();
		}
		invalidate();
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (anim != null) {
			...
			float vFocusNowX = ease(anim.easing, scaleElapsed, anim.vFocusStart.x, anim.vFocusEnd.x - anim.vFocusStart.x, anim
					.duration);
			float vFocusNowY = ease(anim.easing, scaleElapsed, anim.vFocusStart.y, anim.vFocusEnd.y - anim.vFocusStart.y, anim
					.duration);
			...
			// For translate anims, showing the image non-centered is never allowed, for scaling anims it is during the
			// animation.
		...
		if (tileMap != null && isBaseLayerReady()) {
					...
					for (Tile tile : tileMapEntry.getValue()) {
						...
						if (!tile.loading && tile.bitmap != null) {
							...
							if (getRequiredRotation() == ORIENTATION_0) {
								setMatrixArray(dstArray, tile.vRect.left, tile.vRect.top, tile.vRect.right, tile.vRect.top, tile
										.vRect.right, tile.vRect.bottom, tile.vRect.left, tile.vRect.bottom);
							} else if (getRequiredRotation() == ORIENTATION_90) {
								setMatrixArray(dstArray, tile.vRect.right, tile.vRect.top, tile.vRect.right, tile.vRect.bottom,
										tile.vRect.left, tile.vRect.bottom, tile.vRect.left, tile.vRect.top);
							} else if (getRequiredRotation() == ORIENTATION_180) {
								setMatrixArray(dstArray, tile.vRect.right, tile.vRect.bottom, tile.vRect.left, tile.vRect
										.bottom, tile.vRect.left, tile.vRect.top, tile.vRect.right, tile.vRect.top);
							} else if (getRequiredRotation() == ORIENTATION_270) {
								setMatrixArray(dstArray, tile.vRect.left, tile.vRect.bottom, tile.vRect.left, tile.vRect.top,
										tile.vRect.right, tile.vRect.top, tile.vRect.right, tile.vRect.bottom);
							}
						...
						if (tile.visible && debug) {
							canvas.drawText("ISS " + tile.sampleSize + " RECT " + tile.sRect.top + "," + tile.sRect.left + "," +
									tile.sRect.bottom + "," + tile.sRect.right, tile.vRect.left + 5, tile.vRect.top + 15,
									debugPaint);
						}
					}
				}
			...
			if (debug) {
				...
				canvas.drawText("Translate: " + String.format(Locale.ENGLISH, "%.2f", vTranslate.x) + ":" + String.format(Locale
						.ENGLISH, "%.2f", vTranslate.y), 5, 35, debugPaint);
				PointF center = getCenter();
				canvas.drawText("Source center: " + String.format(Locale.ENGLISH, "%.2f", center.x) + ":" + String.format(Locale
						.ENGLISH, "%.2f", center.y), 5, 55, debugPaint);
			...
			if (stateBitmapCallback != null) {
				// create our state bitmaps at the same size as the canvas for easy duplication
				if (mStateBitmap != null && (getWidth() != mStateBitmap.getWidth() || getHeight() != mStateBitmap.getHeight())) {
					stateBitmapCallback.bitmapReady(null);
					// mStateBitmap.recycle(); // this causes crashes if interrupting auto zooming; not needed anyway
					mStateBitmap = null;
				}
				if (mStateBitmap == null) {
					mStateBitmap = Bitmap.createBitmap(getWidth(), getHeight(), Bitmap.Config.RGB_565);
				}
				Canvas canvas2 = new Canvas(mStateBitmap);
				canvas2.drawBitmap(bitmap, matrix, bitmapPaint);
				stateBitmapCallback.bitmapReady(mStateBitmap);
				stateBitmapCallback = null;
			}
		}

		if (mIsColourPickerTouching) {
			// show the colour picker when appropriate
			canvas.save();
			int buttonAdjustment = (int) (mButtonSize * 0.75); // as buttons are in a triangle TODO: could be improved
			float xPosition = mCurrentX;
			float yPosition = mCurrentY - mLensVerticalOffset;
			int width = getWidth();
			int height = getHeight() - mControlButtonHeight + buttonAdjustment;
			int marginRequired = mLensRadius + mButtonSize;
			if (xPosition - marginRequired < 0) {
				xPosition = marginRequired;
			} else if (xPosition + marginRequired > width) {
				xPosition = width - marginRequired;
			}
			if (yPosition - marginRequired < mButtonSize - buttonAdjustment) {
				yPosition = mButtonSize - buttonAdjustment + marginRequired;
			} else if (yPosition + marginRequired > height) {
				yPosition = height - marginRequired;
			}
	...
	public interface StateBitmapCallback {
		void bitmapReady(Bitmap bitmap);
	}

	public void getStateBitmap(StateBitmapCallback callback) {
		stateBitmapCallback = callback;
		invalidate();
	}
	...
	private boolean tileVisible(Tile tile) {
		...
		return !(sVisLeft > tile.sRect.right || tile.sRect.left > sVisRight || sVisTop > tile.sRect.bottom || tile.sRect.top >
				sVisBottom);
	}
	 ...
	 * Minimum scale
	 * is set so one dimension fills the view and the image is centered on the other dimension. Used to calculate what the
	 * target of an
	 * animation should be.
	 *
	 * @param center Whether the image should be centered in the dimension it's too small to fill. While animating this can be
	 *               false to avoid changes in direction as bounds are reached.
	...
	private void fitToBounds(boolean center, ScaleAndTranslate sat) {
		...
		float xPaddingRatio = getPaddingLeft() > 0 || getPaddingRight() > 0 ? getPaddingLeft() / (float) (getPaddingLeft() +
	...
	private void initialiseTileMap(Point maxTileDimensions) {
		...
		while (true) {
			...
			while (subTileWidth + xTiles + 1 > maxTileDimensions.x || (subTileWidth > getWidth() * 1.25 && sampleSize <
					fullImageSampleSize)) {
				xTiles += 1;
				sTileWidth = sWidth() / xTiles;
				subTileWidth = sTileWidth / sampleSize;
			}
			while (subTileHeight + yTiles + 1 > maxTileDimensions.y || (subTileHeight > getHeight() * 1.25 && sampleSize <
					fullImageSampleSize)) {
				yTiles += 1;
			...
			for (int x = 0; x < xTiles; x++) {
				for (int y = 0; y < yTiles; y++) {
					...
					tile.sRect = new Rect(x * sTileWidth, y * sTileHeight, x == xTiles - 1 ? sWidth() : (x + 1) * sTileWidth, y
							== yTiles - 1 ? sHeight() : (y + 1) * sTileHeight);
	...
	private static class TilesInitTask extends AsyncTask<Void, Void, int[]> {
		...
		public TilesInitTask(SubsamplingScaleImageView view, Context context, DecoderFactory<? extends ImageRegionDecoder>
				decoderFactory, Uri source) {
	...
	private static class BitmapLoadTask extends AsyncTask<Void, Void, Integer> {
		...
		public BitmapLoadTask(SubsamplingScaleImageView view, Context context, DecoderFactory<? extends ImageDecoder>
				decoderFactory, Uri source, boolean preview) {
			this.viewRef = new WeakReference<SubsamplingScaleImageView>(view);
			this.contextRef = new WeakReference<Context>(context);
	...
	private int getExifOrientation(String sourceUri) {
		...
		if (sourceUri.startsWith(ContentResolver.SCHEME_CONTENT)) {
			...
			try {
				...
				int orientationAttr = exifInterface.getAttributeInt(ExifInterface.TAG_ORIENTATION, ExifInterface
						.ORIENTATION_NORMAL);
				if (orientationAttr == ExifInterface.ORIENTATION_NORMAL || orientationAttr == ExifInterface
	...
	/**
	 * By default the View automatically calculates the optimal tile size. Set this to override this, and force an upper limit
	 * to the dimensions of the generated tiles. Passing {@link SubsamplingScaleImageView#TILE_SIZE_AUTO} will re-enable the default behaviour.
	 */
	...
	private Rect sourceToViewRect(Rect sRect, Rect vTarget) {
		vTarget.set((int) sourceToViewX(sRect.left), (int) sourceToViewY(sRect.top), (int) sourceToViewX(sRect.right), (int)
	 ...
	 * @return {@link AnimationBuilder} instance. Call {@link SubsamplingScaleImageView.AnimationBuilder#start()} to start the
	 * anim.
	 */
	...
	public final class AnimationBuilder {
		...
		public void start() {
			...
			PointF targetSCenter = panLimited ? limitedSCenter(this.targetSCenter.x, this.targetSCenter.y, targetScale, new
			...
			if (vFocus != null) {
				...
				anim.vFocusEnd = new PointF(vFocus.x + (satEnd.vTranslate.x - vTranslateXEnd), vFocus.y + (satEnd.vTranslate.y -
						vTranslateYEnd));
			}
	...
	// TODO: all of the below is to support the colour lens on the zoomable image view
	// TODO: can it be extended to integrate this, rather than needing a local copy?
	// TODO: (difficulties would be in onZoomOrPanCompleted and around the state bitmap)
	private static final int AVERAGE_RADIUS = 20; // the actual size to average
	private static final int DOUBLE_AVERAGE_RADIUS = AVERAGE_RADIUS * 2;
	private static final int DEFAULT_LENS_RADIUS = 75; // the size of the lens to draw
	private static final int DEFAULT_LENS_OUTLINE_WIDTH = 5;
	private static final int DEFAULT_LENS_VERTICAL_OFFSET = 160;  // to avoid finger occlusion
	private static final int DEFAULT_BUTTON_SIZE = 64;

	private Paint mColourPaint;
	private Paint mOutlinePaint;

	private Bitmap mBitmap;
	private boolean mIsColourPickerTouching = false;
	...
	private PointF mCorrectedTouchPosition = new PointF();

	private int mLensRadius = DEFAULT_LENS_RADIUS;
	private int mLensOutlineWidth = DEFAULT_LENS_OUTLINE_WIDTH;
	private int mLensVerticalOffset = DEFAULT_LENS_VERTICAL_OFFSET;
	private int mButtonSize = DEFAULT_BUTTON_SIZE;
	private int mControlButtonHeight;

	private void initialise(Context context) {
		Resources resources = context.getResources();
		mLensRadius = resources.getDimensionPixelSize(R.dimen.colour_picker_lens_radius);
		mLensOutlineWidth = resources.getDimensionPixelSize(R.dimen.colour_picker_lens_outline_width);
		mLensVerticalOffset = resources.getDimensionPixelSize(R.dimen.colour_picker_lens_vertical_offset);
		mButtonSize = resources.getDimensionPixelSize(R.dimen.colour_picker_button_size) / 2; // half - is radius below
		mControlButtonHeight = resources.getDimensionPixelSize(R.dimen.control_button_height);

		mColourPaint = new Paint();
		mColourPaint.setColor(Color.GRAY);
		mColourPaint.setStyle(Paint.Style.FILL);

		mOutlinePaint = new Paint();
		mOutlinePaint.setColor(Color.DKGRAY);
		mOutlinePaint.setStyle(Paint.Style.STROKE);
		mOutlinePaint.setStrokeWidth(mLensOutlineWidth);
		mOutlinePaint.setAntiAlias(true);
	}

	public void setIsColourPickerTouching(boolean isTouching) {
		if (isTouching != mIsColourPickerTouching) {
			mIsColourPickerTouching = isTouching;
			if (mIsColourPickerTouching) {
				int newColour = getDominantColour(mBitmap);
				mColourPaint.setColor(newColour);
			}
			invalidate();
		}
	}

	public boolean getIsColourPickerTouching() {
		return mIsColourPickerTouching;
	}

	public void setTouchColour(int colour) {
		mColourPaint.setColor(colour);
	}

	public int getTouchColour() {
		return mColourPaint.getColor();
	}

	public void updateColourPickerPosition(float x, float y) {
		mCurrentX = x;
		mCurrentY = y;
		int newColour = getDominantColour(mStateBitmap);
		mColourPaint.setColor(newColour);
		invalidate();
	}

	public PointF getCorrectedTouchPosition() {
		return mCorrectedTouchPosition;
	}
	...
	private int getDominantColour(Bitmap bitmap) {
		...
		int xPoint = Math.max(0, Math.round(mCurrentX - AVERAGE_RADIUS));
		int yPoint = Math.max(0, Math.round(mCurrentY - AVERAGE_RADIUS));
		if (xPoint + DOUBLE_AVERAGE_RADIUS > imageWidth) {
			xPoint = imageWidth - DOUBLE_AVERAGE_RADIUS;
		}
		if (yPoint + DOUBLE_AVERAGE_RADIUS > imageHeight) {
			yPoint = imageHeight - DOUBLE_AVERAGE_RADIUS;
		}

		// getPixels() always returns full rows (other parts are set to zero)
		int[] pixels = new int[imageWidth * DOUBLE_AVERAGE_RADIUS];

		// get the square around the current touch point (we draw a circle, but estimate based on a square...)
		bitmap.getPixels(pixels, 0, imageWidth, xPoint, yPoint, DOUBLE_AVERAGE_RADIUS, DOUBLE_AVERAGE_RADIUS);

		int redBucket = 0;
		int greenBucket = 0;
		int blueBucket = 0;

		for (int y = 0; y < DOUBLE_AVERAGE_RADIUS; y++) {
			for (int x = 0; x < DOUBLE_AVERAGE_RADIUS; x++) {
				int color = pixels[x + (y * imageWidth)]; // x + y * width
		...
		int areaSize = DOUBLE_AVERAGE_RADIUS * DOUBLE_AVERAGE_RADIUS;
		return Color.rgb(redBucket / areaSize, greenBucket / areaSize, blueBucket / areaSize);
	}

	private OnAnimationEventListener mOnAnimationEventListener = new OnAnimationEventListener() {
		@Override
		public void onComplete() {
			checkAndSendZoomOrPanCompleted();
		}
	...
	private void checkAndSendZoomOrPanCompleted() {
		if (onImageEventListener != null && !mIsColourPickerTouching && zoomEnabled && panEnabled) {
			onImageEventListener.onZoomOrPanCompleted();
		}
	}
}
